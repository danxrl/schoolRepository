...
while($msn < $mons){
$FoundIn=array_search($msn, $route);
if ($FoundIn != NULL){
...
if ($selected < 5){
$grafo[$contG]= array($keys->iataCode,$rasult[$route[$FoundIn]+1],$val);
$contG++;
}
else{
	if ($grafo[$keys->iataCode]== Null){
		$grafo[$keys->iataCode]=array();
	array_push($grafo[$keys->iataCode],array($keys->coordinates->latitude,$keys->coordinates->longitude));
	}
	array_push($grafo[$keys->iataCode],array($rasult[$route[$FoundIn]+1]));
}
